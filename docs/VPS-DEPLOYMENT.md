...
# Clawdbot API (separate subdomain, highly restricted)
server {
    listen 443 ssl http2;
    ...
    location /api/ {
        ...
        if ($http_authorization != "Bearer $CLAWDBOT_API_TOKEN") {
            return 401;
        }
...
### 5. Clawdbot Gateway (localhost only)

**Gateway should NOT be exposed publicly.**

**Config:** `~/.config/clawdbot/gateway.json`
...
API_TOKEN=$API_TOKEN
SYNC_PORT=3738
CLAWDBOT_API=http://127.0.0.1:3737
...
const CONFIG = {
  ...
  AI_USER_ID: process.env.AI_USER_ID || 'oracle',
};
...
**Result:** Two Clawdbot instances + you, all in same room, collaborating.
...
- [ ] Clawdbot Gateway on localhost only
...
4. Test connection from local Clawdbot
5. Invite second Clawdbot instance to join
